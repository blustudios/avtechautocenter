...
export default function Fornecedores() {
  ...
  const [form, setForm] = useState({ nome: '', telefone: '', endereco: '', identificacao_extrato: '' });
  ...
  const save = async () => {
    ...
    setShowForm(false);
    setEditId(null);
    setForm({ nome: '', telefone: '', endereco: '', identificacao_extrato: '' });
    fetch();
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Button onClick={() => { setForm({ nome: '', telefone: '', endereco: '', identificacao_extrato: '' }); setEditId(null); setShowForm(true); }}>
      ...
      <div className="space-y-2">
        {filtered.map(f => (
          <div key={f.id} onClick={() => {
            setForm({ nome: f.nome, telefone: f.telefone || '', endereco: f.endereco || '', identificacao_extrato: f.identificacao_extrato || '' });
